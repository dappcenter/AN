...
function refreshData(){
    ...
    contractBalance(function(result){
        ...
        balanceElem.textContent = 'Game Balance: ' + stripDecimals(rawStr, 1) + ' TRX';
    });
    ...
    calculateEggSell(86400,function(sun){
        devFee(sun,function(fee){
            ...
            sellsforexampledoc.textContent='Sell 1 Cr for ' + tronWeb.fromSun(sun) + ' TRX';
        });
    });
    lastHatch(tronWeb.defaultAddress['base58'],function(lh){lastHatchTime=lh});
    EGGS_TO_HATCH_1SHRIMP(function(eggs){eggstohatch1=eggs});
    getMyEggs(function(eggs){
        ...
        if(lastNumShrimp==0){timeuntilfulldoc.textContent='?'}
    });
    getMyShrimp(function(shrimp){
        lastNumShrimp=shrimp
        var gfsdoc=document.getElementById('getfreeshrimp')
        if(shrimp>0){gfsdoc.style.display="none"} else {gfsdoc.style.display="inline-block"}
        var allnumshrimp=document.getElementsByClassName('numshrimp')
        for(var i=0;i<allnumshrimp.length;i++){if(allnumshrimp[i]){allnumshrimp[i].textContent=translateQuantity(shrimp)}}
    ...
    prldoc.textContent="Copy Referral Link"
    var copyText = document.getElementById("copytextthing");
    copyText.value=window.location.origin+"?ref="+tronWeb.defaultAddress['base58']
}
function updateEggNumber(eggs){
    ...
    for(var i=0;i<allnumeggs.length;i++){if(allnumeggs[i]){allnumeggs[i].textContent=translateQuantity(eggs)}}
}

function hatchEggs1(){
    ref=getQueryVariable('ref')
    if (!tronWeb.isAddress(ref)){ref=tronWeb.defaultAddress['base58']}
...
function liveUpdateEggs(){
    if(lastSecondsUntilFull>1 && lastNumEggs>=0 && lastNumShrimp>0 && eggstohatch1>0){
        ...
        if(currentTime/1000-lastHatchTime>eggstohatch1){return;}
...
function updateBuyPrice(){
    ...
    calculateEggBuySimple(suntospend,function(eggs){devFee(eggs,function(fee){eggstobuydoc.textContent=formatEggs(eggs-fee)});});
}
function buyEggs2(){
    var trxspenddoc=document.getElementById('ethtospend')
    suntospend = tronWeb.toSun(trxspenddoc.value)
    buyEggs(suntospend,function(){displayTransactionMessage();});
}

function mineTRC10(){
    var infinityToSpend=document.getElementById('eggstobuy')
    var infinityMineVal=infinityToSpend.value
    mineTRC10(infinityMineVal,function(){displayTransactionMessage();});
}

function formatEggs(eggs){return translateQuantity(eggs/eggstohatch1)}
...
function disableButtons(){
    ...
    for(var i=0;i<allnumshrimp.length;i++){if(allnumshrimp[i]){allnumshrimp[i].style.display="none"}}
    var allnumshrimp=document.getElementsByClassName('btn-md')
    for(var i=0;i<allnumshrimp.length;i++){if(allnumshrimp[i]){allnumshrimp[i].style.display="none"}}
}
function enableButtons(){
    var allnumshrimp=document.getElementsByClassName('btn-lg')
    for(var i=0;i<allnumshrimp.length;i++){if(allnumshrimp[i]){allnumshrimp[i].style.display="inline-block"}}
    var allnumshrimp=document.getElementsByClassName('btn-md')
    for(var i=0;i<allnumshrimp.length;i++){if(allnumshrimp[i]){allnumshrimp[i].style.display="inline-block"}}
}
